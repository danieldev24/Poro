...
import com.apps.daniel.poro.presentation.settings.ProfilePreference.ProfileChangeListener
...
class DurationsSettingsFragment : PreferenceFragmentCompat(), ProfileChangeListener,
    ...
    override fun onDisplayPreferenceDialog(preference: Preference) {
        when {
            preference is ProperSeekBarPreference -> {
                val dialog = ProperSeekBarPreferenceDialog.newInstance(preference.getKey(), this)
                dialog.setTargetFragment(this, 0)
                dialog.show(parentFragmentManager, null)
            }
            preference.key == PreferenceHelper.SAVE_CUSTOM_PROFILE -> {
            ...
            preference.key == PreferenceHelper.PROFILE -> {
